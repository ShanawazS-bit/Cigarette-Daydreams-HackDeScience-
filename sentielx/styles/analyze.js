...
const fs = require("fs");
const { _analyzeInternal: analyze, detectBySyntax } = require(path.join(__dirname, "../dist/index.js"));
...
const SRGB_COLORS = {
    bug: "\x1b[38;2;255;85;85m",         // Bright Red
    security: "\x1b[38;2;255;121;198m",  // Pink/Magenta
    complexity: "\x1b[38;2;255;184;108m",// Orange
    redundancy: "\x1b[38;2;139;233;253m",// Cyan
    style: "\x1b[38;2;80;250;123m",      // Green
};
...
async function run(code, filename) {
    ...
    if (report.issues.length === 0) {
        ...
        console.log(BOLD + YELLOW + "  ISSUES (" + report.issues.length + "):" + RESET);
    ...
    if (report.securityIssues.length > 0) {
        console.log("\n" + DIV);
        console.log(BOLD + YELLOW + "  SECURITY (" + report.securityIssues.length + "):" + RESET);
    ...
    if (report.complexityMetrics.functions.length > 0) {
        console.log("\n" + DIV);
        console.log(BOLD + YELLOW + "  COMPLEXITY FUNCTIONS (" + report.complexityMetrics.functions.length + "):" + RESET);
    ...
    if (report.redundancy.duplicates.length > 0) {
        console.log("\n" + DIV);
        console.log(BOLD + YELLOW + "  REDUNDANCY DUPLICATES (" + report.redundancy.duplicates.length + "):" + RESET);
    ...
    // ── Formatting result ──────────
    if (report.formatting && report.formatting.stats && report.formatting.stats.changed) {
        console.log("\n" + DIV);
        const f = report.formatting;
        console.log(BOLD + YELLOW + "  FORMATTING APPLIED: " + RESET + GREEN + "yes" + RESET);
        console.log("  Additions: " + GREEN + "+" + f.stats.additions + RESET);
        console.log("  Deletions: " + RED + "-" + f.stats.deletions + RESET);
        console.log("\n  " + BOLD + YELLOW + "--- Formatted Code Snippet ---" + RESET);
        console.log("  " + DIM + f.formatted.substring(0, 1000).split("\n").join("\n  ") + RESET);
        if (f.formatted.length > 1000) console.log("  " + DIM + "... (truncated)" + RESET);
    } else if (report.formatting) {
        console.log("\n" + DIV);
        console.log(BOLD + YELLOW + "  FORMATTING APPLIED: " + RESET + DIM + "already well-formatted" + RESET);
    }
    ...
    // ── Stage timing  ───────────────
    console.log("\n" + DIV);
    console.log(BOLD + YELLOW + "  STAGE TIMING:" + RESET);
